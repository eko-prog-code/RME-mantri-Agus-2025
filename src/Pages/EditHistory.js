...
function EditHistory() {
    ...
    const handleFilter = () => {
        const filtered = patients.map(patient => {
            ...
            const filteredRecords = medicalRecords.filter(([recordId, record]) => {
                ...
                const localDate = new Date(record.timestamp).toLocaleDateString('id-ID', {
                    timeZone: 'Asia/Jakarta' // Sesuaikan dengan zona waktu Anda
                });
                return localDate === new Date(filterDate).toLocaleDateString('id-ID', {
    ...
    const handleSaveChanges = async () => {
        if (editPatient) {
            try {
                await axios.put(`https://praktek-mandiri-mantri-agus-default-rtdb.asia-southeast1.firebasedatabase.app/patients/${editPatient.id}.json`, editPatient);
    ...
    return (
        <div className="EditHistory-container">
            ...
            <div className="EditHistory-buttons">
                <button className="add-patient-button" onClick={() => toggleModal('search')}>+Pasien</button>
                <button className="register-patient-button" onClick={() => toggleModal('register')}>+Register</button>
            </div>
                ...
                {filteredPatients.length > 0 ? (
                    filteredPatients.map((patient) => (
                        <li key={patient.id} className="EditHistory-patient-item">
                            <h3>{patient.name}</h3>
                            <p>Usia: {`${patient.usia.years} Tahun, ${patient.usia.months} Bulan, ${patient.usia.days} Hari`}</p>
                            <ul>
                                {Object.entries(patient.medical_records || {}).map(
                                    ([recordId, record]) => (
                                        <li key={recordId}>
                                            <Link
                                                to={`/emr/${patient.id}/update-treatment/${recordId}`}
                                                className="edit-icon-link"
                                            >
                                                <FaEdit className="edit-icon" />
                                            </Link>
                                            <p>Keluhan: {record.complaint}</p>
                                            <p>Pemeriksaan Fisik: {record.condition_physical_examination}</p>
